...
usage() {
  printf 'Usage: %s --dir DIR --samples N [--language LANGUAGE] [--fail-fast]\n' "$0" >&2
  printf 'Runs sourcery-analyzer sample on each git repository directly under DIR.\n' >&2
...
analyze_project() {
  ...
  local cmd=(cargo run -p sourcery-analyzer -- sample "$project_path" "$samples")
...
printf 'Completed sampled analysis for %d project(s); skipped %d non-git directories.\n' "$analyzed" "$skipped"
